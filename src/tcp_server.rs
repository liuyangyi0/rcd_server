...
// 定义一个异步函数 `handle_client`，用于处理TCP客户端的请求和数据传输
// 接收三个参数：`framed`，`global_sender`，`tx`，并返回一个 `io::Result<()>` 类型的结果
// tx 用于 发送数据给串口线程
pub async fn handle_client(mut framed: Framed<TcpStream, LengthDelimitedCodec>,
...
pub async fn handle_client_1(mut framed: Framed<TcpStream, LengthDelimitedCodec>,
                           global_sender: Arc<TokioMutex<Vec<Arc<tokio_mpsc::Sender<HashMap<String, Value>>>>>>,
                             serial_prots:Vec<SerialPortConfig>,
...
// 定义一个异步函数 `run_tcp_server`，接收三个参数：`tx`, `global_sender`，并返回一个 `io::Result<()>` 类型的结果
pub async fn run_tcp_server(tx: mpsc::Sender<Command>,
    ...
    loop {
        ...
        match listener.accept().await {
            ...
            Ok((stream, _addr)) => {
                ...
                tokio::spawn(handle_client(framed, global_sender_clone, tx.clone()));
            },
            // 如果接受连接失败，则输出错误信息
            Err(e) => {
...
pub async fn run_tcp_server_1(serial_prots:Vec<SerialPortConfig>,
                            txs: Vec<mpsc::Sender<Command>>,
                            global_sender: Arc<TokioMutex<Vec<Arc<tokio_mpsc::Sender<HashMap<String, Value>>>>>>,) -> io::Result<()> {
    // 绑定一个TCP监听器到本地的8080端口
    let listener = TcpListener::bind("127.0.0.1:11002").await?;
    ...
    loop {
        ...
        match listener.accept().await {
            ...
            Ok((stream, _addr)) => {
                ...
                let framed = Framed::new(stream, LengthDelimitedCodec::new());
                // 异步启动一个新的任务来处理客户端，传递处理好的帧和克隆的状态
                tokio::spawn(handle_client_1(framed, global_sender_clone, serial_prots.clone(), txs.clone()));
            },
            // 如果接受连接失败，则输出错误信息
            Err(e) => {
                eprintln!("Failed to accept connection: {:?}", e);
